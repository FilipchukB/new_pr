...
class Level4:
    def __init__(self, screen, settings):
        ...
        self.question = pygame.image.load('level_4.jpg')
    ...
    def pr_next(self):
        ...
        if self.settings.current_level == 4 and mouse[0] < self.screen_rect.centerx and mouse[
            1] > self.screen_rect.centery:
            if click[0] == 1:
                self.settings.current_level = 0
                self.settings.histori_4 = 1
                pygame.mixer.Sound.play(button_sound)
                pygame.time.delay(200)
        elif self.settings.current_level == 4 and mouse[0] > self.screen_rect.centerx and mouse[
            1] > self.screen_rect.centery:
            print(click[0])
            if click[0] == 1:
                print('qqqqqqqq')
                self.settings.current_level = 5
                pygame.mixer.Sound.play(button_sound)
                pygame.time.delay(200)
        if self.settings.current_level == 4:
