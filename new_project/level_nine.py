...
class Level9:
    def __init__(self, screen, settings):
        ...
        self.question = pygame.image.load('level_9.jpg')
    ...
    def pr_next(self):
        ...
        if self.settings.current_level == 9 and mouse[0] < self.screen_rect.centerx and mouse[
            1] > self.screen_rect.centery:
            if click[0] == 1:
                self.settings.current_level = 10
                self.settings.histori_5 = 1
                pygame.mixer.Sound.play(button_sound)
                pygame.time.delay(200)
                sys.exit()
        elif self.settings.current_level == 9 and mouse[0] > self.screen_rect.centerx and mouse[
            1] > self.screen_rect.centery:
            print(click[0])
            if click[0] == 1:
                print('qqqqqqqq')
                ...
                pygame.mixer.Sound.play(button_sound)
                pygame.time.delay(200)
        if self.settings.current_level == 9:
