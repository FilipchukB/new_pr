...
class Level7:
    def __init__(self, screen, settings):
        ...
        self.question = pygame.image.load('level_7.jpg')
    ...
    def pr_next(self):
        ...
        if self.settings.current_level == 7 and mouse[0] < self.screen_rect.centerx and mouse[
            1] > self.screen_rect.centery:
            if click[0] == 1:
                self.settings.current_level = 8
                pygame.mixer.Sound.play(button_sound)
                pygame.time.delay(200)
                sys.exit()
        elif self.settings.current_level == 7 and mouse[0] > self.screen_rect.centerx and mouse[
            1] > self.screen_rect.centery:
            print(click[0])
            if click[0] == 1:
                print('qqqqqqqq')
                self.settings.current_level = 8
                self.settings.histori_7 = 1
                pygame.mixer.Sound.play(button_sound)
                pygame.time.delay(200)
